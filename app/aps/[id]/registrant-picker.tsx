...
export type RegistrantPickerItem = {
  id: string;
...
function displayName(r: RegistrantPickerItem) {
...
export default function RegistrantPicker({
  ...
  value: string;
  onChange: (registrantId: string) => void;
  placeholder?: string;
  disabled?: boolean;
}) {
  ...
  const selected = useMemo(
    () => registrants.find((r) => r.id === value) ?? null,
    [registrants, value]
  );
  ...
  const filtered = useMemo(() => {
    ...
    return registrants.filter((r) => {
      ...
      return (
        name.includes(q) || email.includes(q) || (company && company.includes(q))
      );
    });
  }, [registrants, query]);
  ...
  useEffect(() => {
    ...
    function updatePosition() {
      ...
      const top = shouldFlip
        ? Math.max(padding, rect.top - padding - maxHeight - 44)
        : rect.bottom + padding;
              ...
              <div style={{ maxHeight: popoverStyle.maxHeight }} className='overflow-y-auto'>
                {filtered.length === 0 ? (
                  <div className='p-3 text-sm text-slate-600'>
                    No registrants match “{query.trim()}”.
                  </div>
                ) : (
                  filtered.map((r) => (
                    <button
